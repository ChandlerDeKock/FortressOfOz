...
public class Grid {
	...
	public void populateGrid() {
		...
		for(int i = 0 ; i < 10 ; i++){
			...
			if (grid[generatorX][generatorY] instanceof Floor && check <= 0.1) {

				BadThing b = new BadThing();
				Location loc = new Location(generatorX, generatorY);
				b.init(game, this, loc);
				setObjectAt(loc,b);

			}
			if (grid[generatorX][generatorY] instanceof Floor && check >= 0.99) {

				GoodThing g = new GoodThing();
				Location loc = new Location(generatorX, generatorY);
				g.init(game, this, loc);
				setObjectAt(loc,g);
			}
		}
	}
	...
	public void moveAll() {
		...
		for (int i = 0; i < NumRows; i++) {
			for (int j = 0; j < NumCols; j++) {
				if (grid[i][j] != null) {
					GameObject obj = grid[i][j];
					Location oldLoc = new Location(i, j);
					Location newLoc = obj.move();

					if (!newLoc.equals(oldLoc)) {
						Floor f = new Floor();
						f.init(game, this, oldLoc);
						grid1[i][j] = f;
						grid1[newLoc.getRow()][newLoc.getCol()] = obj.collision(grid1[newLoc.getRow()][newLoc.getCol()]);
					}
			...
			for (int j = atY - 5; j < atY + 5 && j < NumCols + atY + 5; j++) {
				...
				if (i >= 0 && i < NumCols && j >= 0 && j < NumRows) {
					...
					if (grid[j][i] != null) {
						...
						grid[j][i].draw(i - (atX - 7), j - (atY - 5),GridCellSize);
	...
	public void updatePathToPlayer() {
		...
		while (!q.isEmpty()) {
			...
			if (grid[nodeLoc.getRow()][nodeLoc.getCol()] instanceof BadThing) {
				...
				p = n;
				if(p.getLocation() != null && p.getPrev() !=null){
					p = p.getPrev();
					bad = p.getLocation();
				}
				break;
